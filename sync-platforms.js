// sync-platforms.js — esegue "cap sync" su TUTTE le piattaforme (Android + iOS)
// e corregge il server.url bundled per ciascuna, perché il loopback dell'host va
// raggiunto in modo diverso a seconda dell'emulatore/simulatore:
...
// "cap sync" copia SEMPRE capacitor.config.json (radice) as-is in entrambe le
// piattaforme: senza questa correzione, l'app iOS erediterebbe 10.0.2.2 e non
// riuscirebbe a raggiungere il server locale (10.0.2.2 non esiste fuori dalla
// NAT dell'emulatore Android).
const fs = require('fs');
...
console.log('[SYNC] cap sync (Android + iOS)...');
...
execFileSync(process.execPath, [capBin, 'sync'], { stdio: 'inherit' });

for (const platform of Object.keys(PLATFORM_SERVER_URL)) {
...
console.log('[SYNC] Completato: Android e iOS allineati a www/ con il server.url corretto per piattaforma.');
